...
class PlayerCameraManager extends hz.Component<typeof PlayerCameraManager> {
  ...
  preStart(): void {
    this.connectNetworkBroadcastEvent(
      CameraManagerEvents.OnRegisterPlayerCamera,
      ({ ObjectId, Object }) => {
  ...
  assignPlayerCamera(): void {
    ...
    while (playerAttemptIndex >= 0) {
      ...
      if (
        this.assignCameraAttempts[playerAttemptIndex].attempts >=
        this.maxAssignAttempts
      ) {
        console.error(
          "Failed to assign a camera to " +
            player.name.get() +
            " after " +
            this.maxAssignAttempts +
            " attempts. Giving up."
  ...
  attemptAssignCamera(player: hz.Player): boolean {
    ...
    if (playerCamera === undefined) {
      ...
      console.warn(
        "Attempted to assign a camera to " +
          player.name.get() +
          " but there was no camera for this player index (Player index: " +
          player.index.get() +
          "). Retrying..."
      );
      console.log(
        "Hint: Check you have enough PlayerCamera objects in the world."
      );
      return false;
      ...
      console.error(
        "Attempted to assign a camera to " +
          player.name.get() +
          " but it is already assigned to " +
          playerCamera.owner.get().name.get() +
          ". Error with player.index."
      );
  ...
  unassignPlayerCamera(player: hz.Player): void {
    this.playerCameras[player.index.get()].owner.set(
