...
export const PlayerCameraEvents = {
  ...
  SetCameraFixedPosition: new hz.NetworkEvent<{
    position: hz.Vec3;
    rotation?: hz.Quaternion;
    duration?: number;
    easing: Easing;
  }>("SetCameraFixedPosition"),
  SetCameraFixedPositionWithEntity: new hz.NetworkEvent<{
    entity: hz.Entity;
    duration?: number;
    easing?: Easing;
  }>("SetCameraFixedPositionWithEntity"),
  SetCameraAttachWithTarget: new hz.NetworkEvent<{
    target: hz.Entity | hz.Player;
    positionOffset?: hz.Vec3;
    rotationOffset?: hz.Vec3 | hz.Quaternion;
  }>("SetCameraAttachWithPlayer"),
  SetCameraPan: new hz.NetworkEvent<{
    positionOffset?: hz.Vec3;
    translationSpeed?: number;
  }>("SetCameraPan"),
  SetCameraCollisions: new hz.NetworkEvent<{ collisionsEnabled: boolean }>(
    "SetCameraCollisions"
  ),
  RevertPlayerCamera: new hz.NetworkEvent<{}>("RevertPlayerCamera"),
  OnCameraResetPressed: new hz.NetworkEvent<{ player: hz.Player }>(
...
export class PlayerCamera extends hz.Component<typeof PlayerCamera> {
  ...
  initCameraListeners(player: hz.Player) {
    ...
    this.connectNetworkEvent(
      player,
      PlayerCameraEvents.SetCameraMode,
      ({ mode }) => {
    ...
    this.connectNetworkEvent(
      player,
      PlayerCameraEvents.SetCameraFixedPosition,
      ({ position, rotation, duration, easing }) => {
    ...
    this.connectNetworkEvent(
      player,
      PlayerCameraEvents.SetCameraPan,
      ({ positionOffset, translationSpeed }) => {
        this.setCameraPan(positionOffset, translationSpeed);
      }
    );
    this.connectNetworkEvent(
      player,
      PlayerCameraEvents.SetCameraCollisions,
      ({ collisionsEnabled }) => {
    ...
    this.connectNetworkEvent(
      player,
      PlayerCameraEvents.RevertPlayerCamera,
      () => {
  ...
  receiveOwnership(
    _serializableState: hz.SerializableState,
    _oldOwner: hz.Player,
    _newOwner: hz.Player
  ): void {
  ...
  setCameraMode(mode: CameraMode) {
    if (mode === CameraMode.Fixed) {
      console.warn(
        "Used SetCameraMode with a fixed camera. Use SetFixedCameraPosition instead."
      );
      return;
    }
    ...
    if (
      this.getPreviousCameraMode() === CameraMode.Fixed ||
      this.getPreviousCameraMode() === CameraMode.Attach
    ) {
    ...
    switch (mode) {
      ...
      case CameraMode.FirstPerson:
        LocalCamera.setCameraModeFirstPerson({
          ...cameraOptions,
          ...cameraOptionsFirstPerson,
        });
        break;
      case CameraMode.ThirdPerson:
        LocalCamera.setCameraModeThirdPerson({
          ...cameraOptions,
          ...cameraOptionsThirdPerson,
        });
        break;
      case CameraMode.Pan:
        LocalCamera.setCameraModePan({ ...cameraOptions, ...cameraOptionsPan }); // uses default pan position. To use custom pan position use setCameraPan
        break;
      default:
  ...
  setCameraFixedPosition(
    position: hz.Vec3,
    rotation: hz.Quaternion | undefined,
  ...
  onCameraResetButtonPressed() {
    if (this.player !== undefined && this.player !== null) {
      ...
      this.sendNetworkEvent(
        this.player,
        PlayerCameraEvents.OnCameraResetPressed,
        { player: this.player }
      ); // You can remove this if you are not using the cutscene system or don't care about the player's camera resetting.
      this.revertPlayerCamera();
    }
  }
  ...
  revertPlayerCamera() {
    if (this.player !== undefined && this.player !== null) {
      ...
      this.sendNetworkEvent(this.player, PlayerCameraEvents.SetCameraMode, {
      ...
      if (this.defaultCameraCollisionsEnabled !== undefined) {
        this.sendNetworkEvent(
          this.player,
          PlayerCameraEvents.SetCameraCollisions,
          { collisionsEnabled: this.defaultCameraCollisionsEnabled }
        );
      }
    } else {
      console.warn(
        "PlayerCamera: revertPlayerCamera called with no player set."
      );
    }
  }
